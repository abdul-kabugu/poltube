...
export default function LibPostCard(video) {
  const [isDisplayDots, setIsDisplayDots] = useState(false)
  const [currentTime, setCurrentTime] = useState(new Date());
  const currentDate = new Date();
  
  const postCreatedAt = new Date(video?.video?.video?.createdAtTime);
...
const duration = moment.duration(diffInHours, 'hours');
  ...
  console.log("the lib video", video)
  ...
  return (
    ...
    >
       ...
       <Box bg="black"  rounded="md" onClick={() => saveToLibrary(video)}>
        <Img  src={`${IPFS_GATEWAY}${video?.video?.video?.image}`} 
          w="100%" h={{base : 190, md : 170}} objectFit="cover" rounded="md" minW={{base : "100%"}}
        />
        </Box>
        </Link>
       <Box mt={2} display="flex"  px={2} alignItems="center" gap={2}>
         <Link to={`/channels/${video?.video?.video?.createdByAccount?.id}`}>
         <Avatar size="xs"   />
         </Link>
         <Link to={`/watch/${video?.video?.id}`}>
         <Text fontSize="sm" fontWeight="semibold">{video?.video?.video?.title}</Text>
         </Link>
        ...
        <HStack gap={3} px={6} ml={4} mt={2}>
         <Text fontSize="sm" color="gray.400">likes {video?.video?.video?.upvotesCount}</Text>
